...
void solve() {
    ...
    cin >> n;
    unsigned long long sum = 0, lower = 1, upper = 9, sub = 0; 
    int md = 998244353;
    int inv = 499122177;
    while(true){
        bool flag = false;
        ...
        unsigned long long next = (((upper - sub) * (upper - sub + 1) * inv) % md - ((lower - sub - 1) * (lower - sub) * inv) % md) % md;
        sum = (sum + next) % md;
        lower *= 10; 
        upper = upper * 10 + 9;
        sub = sub * 10 + 9; 
        //print("debug; " << sum << " " << next); 
        if(flag)break;
    } 
    print(sum);
}
