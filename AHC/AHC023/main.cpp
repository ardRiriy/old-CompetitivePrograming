...
vector<vector<int>> south_water_route(
    h - 1, vector<int>(w));  // そのマスの座標(i, j)の下に水路があるかどうか
vector<vector<int>> east_water_route(
    h, vector<int>(w - 1));  // そのマスの右に水路があるかどうか
    ...
    rep(i, h) {
        rep(j, w) {
            rep(r, 4) {
                ...
                int new_w = j + dx[r];
                if (bd[new_h][new_w] == -1)
                    res[i * w + j].push_back(new_h * w + new_w);
            }
        }
    }
    return res;
...
bool is_placable(Pos p, vector<int> &v, int proceed_day, vector<vector<int>> &bd) {
    if (bd[p.h][p.w] != -1 || depth[p.h][p.w] == INF ||
        (p.h == enter && p.w == 0))
        return false;
    ...
    rep(i, 4) {
        if (is_through(p, i)) {
            ...
            if (bd[nh][nw] == -1)
                ...
                value += abs(sd[crop_num - 1][1] - sd[bd[nh][nw] - 1][1]);
        }
    }
    return value;
}
